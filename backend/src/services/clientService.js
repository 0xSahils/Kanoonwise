...
const createOrUpdateClientProfile = async (userId, profileData) => {
    const existingProfile = await ClientProfile.findOne({ where: { user_id: userId } });
...
const searchLawyers = async (filters = {}, page = 1, limit = 10) => {
    ...
    if (filters.specialization) {
        whereClause.specialization = { [Op.contains]: [filters.specialization] };
    }
    
    if (filters.language) {
        whereClause.languages = { [Op.contains]: [filters.language] };
    }
    ...
    if (filters.min_fee || filters.max_fee) {
        if (filters.min_fee) {
            feeFilter['fee_structure.consultation'] = { [Op.gte]: filters.min_fee };
        }
        if (filters.max_fee) {
            feeFilter['fee_structure.consultation'] = { 
                ...feeFilter['fee_structure.consultation'],
                [Op.lte]: filters.max_fee 
            };
        }
        whereClause[Op.and] = [
            sequelize.where(
    ...
    for (let lawyer of lawyers.rows) {
        ...
        const reviewStats = await Review.findOne({
            ...
            attributes: [
                [sequelize.fn('COALESCE', sequelize.fn('AVG', sequelize.col('rating')), 0), 'average_rating'],
        ...
        if (filters.min_rating && (!reviewStats?.average_rating || reviewStats.average_rating < filters.min_rating)) {
            // Skip this lawyer if it doesn't meet minimum rating
            continue;
        }

        // Add review stats to lawyer data
        lawyer.dataValues.average_rating = parseFloat(reviewStats?.average_rating || 0);
...
const bookAppointment = async (clientId, appointmentData) => {
    ...
    // Get client profile
    const clientProfile = await ClientProfile.findOne({ where: { id: clientId } });
    if (!clientProfile) {
        throw new Error('Client profile not found');
    }

    const fee = appointmentData.consultation_type === 'online' ? 
...
const getClientAppointments = async (clientId) => {
    return await Appointment.findAll({
        ...
        include: [
            {
                model: LawyerProfile,
                attributes: ['full_name', 'specialization', 'fee_structure', 'city', 'years_experience'],
                include: [
                    {
                        model: User,
                        attributes: ['email']
